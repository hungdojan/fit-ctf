...
def test_cli_create_user(cli_data: CLIData):
    ...
    assert LocalAuth(ctf_app.user_mgr).validate_credentials(data[0], data[1])
...
def test_cli_create_multiple_users(cli_data: CLIData):
    ...
    with tempfile.NamedTemporaryFile("w+") as tf:
        ...
        assert len(ctf_app.user_mgr.get_docs()) == 5

        f = StringIO(result.output)
        for user_data in list(csv.reader(f))[1:]:
            LocalAuth(ctf_app.user_mgr).validate_credentials(user_data[0], user_data[1])
...
def test_cli_list_users(cli_data: CLIData):
    ...
    assert len(data) == 3
    for d in data:
        assert ctf_app.user_mgr.get_doc_by_filter(username=d[0])
...
def test_cli_get_user_info(cli_data: CLIData):
    ...
    user = ctf_app.user_mgr.get_user(data["username"])
...
def test_cli_enrolled_projects(cli_data: CLIData):
    ...
    user, _ = ctf_app.user_mgr.create_new_user("user4", "strongPassword1")
...
def test_cli_delete_user(cli_data: CLIData):
    ...
    assert ctf_app.user_mgr.get_user("user2")
