...
def cancel_from_project(ctx: click.Context, username: str, project_name: str):
    ...
    ctf_app: CTFApp = ctx.parent.obj["ctf_app"]  # pyright: ignore
    try:
        asyncio.run(ctf_app.enroll_mgr.cancel_enrollment(username, project_name))
    except CTFBaseException as e:
        click.echo(e)
        ctx.exit(1)
...
def cancel_user(ctx: click.Context, username: str):
    ...
    ctf_app: CTFApp = ctx.parent.obj["ctf_app"]  # pyright: ignore
    try:
        asyncio.run(ctf_app.enroll_mgr.cancel_user_from_all_projects(username))
    except CTFBaseException as e:
        click.echo(e)
        ctx.exit(1)
...
def cancel_project(ctx: click.Context, project_name: str):
    ...
    ctf_app: CTFApp = ctx.parent.obj["ctf_app"]  # pyright: ignore
    try:
        asyncio.run(ctf_app.enroll_mgr.cancel_all_project_enrollments(project_name))
    except CTFBaseException as e:
        click.echo(e)
        ctx.exit(1)
