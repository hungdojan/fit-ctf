...
async def test_login(tui_app: App, connected_data: FixtureData):
    ...
    async with tui_app.run_test() as pilot:
        ...
        assert (
            tui_app.screen.query_one("#login-message-label", Label).styles.visibility == "visible"
        )
        ...
        user = ctf_app.user_mgr.get_user("user1")
        assert user and user.sessions and user.sessions[-1].state == LoginSession.State.LOGIN
