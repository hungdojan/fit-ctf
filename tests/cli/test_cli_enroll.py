...
def test_cli_enroll_user(cli_data: CLIData):
    ...
    assert len(ctf_app.enroll_mgr.get_enrollments_for_project("prj1")) == 2
    cmd = "enrollment enroll -u user1 -pn prj1".split()
    result = cli_runner.invoke(cli, cmd)
    assert result.exit_code == 0
    assert re.search("was enrolled", result.output)
    assert len(ctf_app.enroll_mgr.get_enrollments_for_project("prj1")) == 3
...
def test_cli_cancel_enrollment(cli_data: CLIData):
    ...
    result = cli_runner.invoke(cli, cmd)
    assert result.exit_code == 0
    assert len(ctf_app.enroll_mgr.get_enrollments_for_project("prj1")) == 1
...
def test_cli_cancel_user(cli_data: CLIData):
    ...
    result = cli_runner.invoke(cli, cmd)
    assert result.exit_code == 0
    assert len(ctf_app.enroll_mgr.get_enrolled_projects("user2")) == 0
...
def test_cli_cancel_project(cli_data: CLIData):
    ...
    result = cli_runner.invoke(cli, cmd)
    assert result.exit_code == 0
    assert len(ctf_app.enroll_mgr.get_enrollments_for_project("prj1")) == 0
