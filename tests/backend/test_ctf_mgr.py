...
def test_export_project(connected_data: FixtureData):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            data = YamlParser.load_data_file(pathlib.Path(tmp_dir) / "database_dump.yaml")
...
def test_import_project_skips_failed_user_insert(connected_data: FixtureData):
    ...
    with patch.object(ctf_app.user_mgr, "create_and_insert_doc", side_effect=insert_fail_user1):
        ctf_app.import_project(zip_path)
...
def test_import_project_reverts_user_on_enrollment_failure(connected_data: FixtureData):
    ...
    with patch.object(ctf_app.enroll_mgr, "import_enrollment", side_effect=enroll_fail_user2):
        ctf_app.import_project(zip_path)
...
def test_setup_skips_user_after_creation_failure(empty_data: FixtureData):
    ...
    with patch.object(ctf_app.user_mgr, "create_new_user", side_effect=create_fail_user2):
...
async def test_setup_env_from_file(empty_data: FixtureData):
    ...
    assert not ctf_app.setup_env_from_file(fixture_path() / "user_data.yaml", dry_run=True)
